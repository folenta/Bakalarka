...
def main(yolo):
    ...
    props_file = open('cam_props/' + args.video + '_prop.txt', 'r')
    roi_file = open('dataset/ROIs/' + args.video + '.txt', 'r')
    out_list = open('outlist/' + args.video + '_out.txt', 'w+')  # file for text output
    out_list.write("video_clip_id, frame_id, movement_id, vehicle_class_id")  # first line in text output
    ...
    video_capture = cv2.VideoCapture('dataset/Dataset_A/' + args.video + '.mp4')
    ...
    if writeVideo_flag:
        ...
        out = cv2.VideoWriter('outputs/' + args.video + '.avi', fourcc, 15, (w, h))
